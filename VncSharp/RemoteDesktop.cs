...
namespace VncSharp
{
    ...
    public sealed class RemoteDesktop : Panel
    {
        ...
        private readonly Image _designModeDesktop; // Used when painting control in VS.NET designer

        private Bitmap _desktop; // Internal representation of remote image.
        private VncDesktopTransformPolicy _desktopPolicy;
        ...
        private int _port = 5900; // The port to connect to on remote host (5900 is default)

        private KeyboardHook.ModifierKeys _previousModifierKeyState;
        private RuntimeState _state = RuntimeState.Disconnected;
        private VncClient _vnc; // The Client object handling all protocol-level interaction
        ...
        public RemoteDesktop()
        {
            ...
            AutoScroll = _desktopPolicy.AutoScroll;
            AutoScrollMinSize = _desktopPolicy.AutoScrollMinSize;
        ...
        {
            get { return _state == RuntimeState.Connected; }
        }
        ...
        {
            get { return _vnc == null ? "Disconnected" : _vnc.HostName; }
        }
        ...
        public bool Scaled
        {
            get { return _desktopPolicy is VncScaledDesktopPolicy; }
        ...
        private void InsureConnection(bool connected)
        {
            ...
            if (connected)
            {
                ...
                Assert(_state == RuntimeState.Connected ||
            ...
            else
            {
                // disconnected
#if DEBUG
                Assert(_state == RuntimeState.Disconnected,
                    $"RemoteDesktop must be in RuntimeState.Disconnected before calling {methodName}.");
#endif
                if (_state != RuntimeState.Disconnected && _state != RuntimeState.Disconnecting)
                    throw new InvalidOperationException(
        ...
        private void VncUpdate(object sender, VncEventArgs e)
        {
            ...
            Invalidate(_desktopPolicy.AdjustUpdateRectangle(e.DesktopUpdater.UpdateRectangle));

            if (_state != RuntimeState.Connected) return;

            // Make sure the next screen update is incremental
            _vnc.FullScreenRefresh = false;
        }
        ...
        private void SetScalingMode(bool scaled)
        {
            if (InvokeRequired)
            {
                _syncContext.Send(x => SetScalingMode(scaled), null);
                return;
            }

            if (scaled)
                _desktopPolicy = new VncScaledDesktopPolicy(_vnc, this);
            else
                _desktopPolicy = new VncClippedDesktopPolicy(_vnc, this);
        ...
        private void Initialize()
        {
            if (InvokeRequired)
            {
                _syncContext.Send(x => Initialize(), null);
                return;
            }

            // Finish protocol handshake with host now that authentication is done.
            InsureConnection(false);
            _vnc.Initialize(BitsPerPixel, Depth);
        ...
        private void SetupDesktop()
        {
            ...
            _desktop = new Bitmap(_vnc.Framebuffer.Width, _vnc.Framebuffer.Height, PixelFormat.Format32bppPArgb);
        ...
        {
            Assert(_desktop != null, "Can't draw on desktop when null.");
            // Draw the given message on the local desktop
            using (var g = Graphics.FromImage(_desktop))
            {
                g.FillRectangle(Brushes.Black, _vnc.Framebuffer.Rectangle);
                ...
                g.DrawString(message,
                    ...
                    new PointF(_vnc.Framebuffer.Width / 2, _vnc.Framebuffer.Height / 2), format);
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (!DesignMode)
            {
                switch (_state)
                {
                    case RuntimeState.Connected:
                        if (_desktop != null)
                            DrawDesktopImage(_desktop, pe.Graphics);
                        break;
                    case RuntimeState.Disconnected:
                    ...
                    default:
                        ...
                        throw new NotImplementedException($"RemoteDesktop in unknown State: {_state}.");
                }
            }
            else
            {
                // Draw a static screenshot of a Windows desktop to simulate the control in action
                if (_designModeDesktop != null)
                    DrawDesktopImage(_designModeDesktop, pe.Graphics);
            }
        ...
        {
            g.DrawImage(desktopImage, _desktopPolicy.RepositionImage(desktopImage));
        }
        ...
        private void UpdateRemotePointer()
        {
            ...
            var adjusted = _desktopPolicy.GetMouseMoveRectangle();
        ...
        {
            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.LeftShift) !=
                (modifierKeys & KeyboardHook.ModifierKeys.LeftShift))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Shift_L,
                    (modifierKeys & KeyboardHook.ModifierKeys.LeftShift) != 0);
            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.RightShift) !=
                (modifierKeys & KeyboardHook.ModifierKeys.RightShift))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Shift_R,
                    (modifierKeys & KeyboardHook.ModifierKeys.RightShift) != 0);

            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.LeftControl) !=
                (modifierKeys & KeyboardHook.ModifierKeys.LeftControl))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Control_L,
                    (modifierKeys & KeyboardHook.ModifierKeys.LeftControl) != 0);
            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.RightControl) !=
                (modifierKeys & KeyboardHook.ModifierKeys.RightControl))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Control_R,
                    (modifierKeys & KeyboardHook.ModifierKeys.RightControl) != 0);

            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.LeftAlt) !=
                (modifierKeys & KeyboardHook.ModifierKeys.LeftAlt))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Alt_L, (modifierKeys & KeyboardHook.ModifierKeys.LeftAlt) != 0);
            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.RightAlt) !=
                (modifierKeys & KeyboardHook.ModifierKeys.RightAlt))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Alt_R, (modifierKeys & KeyboardHook.ModifierKeys.RightAlt) != 0);

            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.LeftWin) !=
                (modifierKeys & KeyboardHook.ModifierKeys.LeftWin))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Super_L, (modifierKeys & KeyboardHook.ModifierKeys.LeftWin) != 0);
            if ((_previousModifierKeyState & KeyboardHook.ModifierKeys.RightWin) !=
                (modifierKeys & KeyboardHook.ModifierKeys.RightWin))
                _vnc.WriteKeyboardEvent(RfbProtocol.XK_Super_R,
                    (modifierKeys & KeyboardHook.ModifierKeys.RightWin) != 0);

            _previousModifierKeyState = modifierKeys;
        }
        ...
        private bool HandleKeyboardEvent(int msg, int virtualKey, KeyboardHook.ModifierKeys modifierKeys)
        {
            ...
            if (modifierKeys != _previousModifierKeyState)
            ...
            _vnc.WriteKeyboardEvent(Convert.ToUInt32(TranslateVirtualKey(virtualKey, modifierKeys)), pressed);
        ...
        private void PressKeys(uint[] keys, bool release)
        {
            ...
            foreach (var u in keys)
                _vnc.WriteKeyboardEvent(u, true);
