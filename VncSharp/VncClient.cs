...
namespace VncSharp
{
    ...
    public class VncClient
    {
        ...
        {
            return Connect(host, display, port, _viewOnlyMode);
        }
            ...
            try
            {
                ...
                if (types[0] == 0)
                    throw new VncProtocolException(
                        ...
                        _rfb.ReadSecurityFailureReason());
                _securityType = GetSupportedSecurityType(types);
                Debug.Assert(_securityType > 0, "Unknown Security Type(s)",
...
//					|| types[i] == 18 	// TLS
        ...
        public bool Authenticate(string password)
        {
            ...
            if (_rfb.ReadSecurityResult() == 0) return true;
        ...
        public void Initialize(int bitsPerPixel, int depth)
        {
            ...
            _rfb.WriteClientInitialisation(true); // Allow the desktop to be shared
            Framebuffer = _rfb.ReadServerInit(bitsPerPixel, depth);

            _rfb.WriteSetEncodings(new uint[]
            {
            ...
            _rfb.WriteSetPixelFormat(Framebuffer); // set the required ramebuffer format
                    ...
                    switch (_rfb.ReadServerMessageType())
                    {
                        case RfbProtocol.FRAMEBUFFER_UPDATE:
                            ...
                            for (var i = 0; i < rectangles; ++i)
                            {
                                ...
                                _rfb.ReadFramebufferUpdateRectHeader(out var rectangle, out var enc);
...
// There is no managed way to get a system beep (until Framework v.2.0). So depending on the platform, something external has to be called.
        ...
        public void RequestScreenUpdate(bool refreshFullScreen)
        {
            try
            {
                _rfb.WriteFramebufferUpdateRequest(0, 0, (ushort) Framebuffer.Width, (ushort) Framebuffer.Height,
                    !refreshFullScreen);
            }
            catch
            {
